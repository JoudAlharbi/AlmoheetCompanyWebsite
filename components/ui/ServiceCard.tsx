...
export default function ServiceCard({
  ...
  const isIconFocused = service.slug === "graphic-design";
  ...
  return (
    <ScrollReveal className="h-full">
      <article className="group relative flex h-full min-h-[220px] flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:border-brand-blue/25 hover:shadow-soft dark:border-slate-800 dark:bg-slate-900/50 dark:hover:border-brand-blue/35 md:p-7">
        <div
          className={`mb-5 flex shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-brand-blue/10 to-brand-gold/10 text-brand-blue ring-1 ring-brand-blue/10 transition-all duration-300 group-hover:scale-105 group-hover:from-brand-blue/15 group-hover:to-brand-gold/15 group-hover:ring-brand-blue/20 dark:from-brand-blue/20 dark:to-brand-gold/10 ${
            isIconFocused ? "h-16 w-16" : "h-14 w-14"
          }`}
        >
          <ServiceIcon
            name={service.icon}
            className={isIconFocused ? "h-8 w-8" : "h-7 w-7"}
          />
        </div>
